...
1.  **JSON Report**: Located in `reports/docs/evaluations/eval_report_<timestamp>.json`. This contains the raw agent response, the judge's justification for every score, and the pass/fail status per sample.
