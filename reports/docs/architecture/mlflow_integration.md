...
- **Live Monitoring (Runtime):** Every agent assessment is scored by a `monitor_node` and logged to the `acras_live_monitoring` experiment.
...
## 5. Live Monitoring & Feedback Loop (`acras_live_monitoring`)

Following the **"Monitor-as-a-Node"** pattern, ACRAS integrates qualitative evaluation directly into the agent reasoning graph. This enables real-time auditing of every production report generated by the agent.

### 5.1 Orchestration — `monitor_node`

The `monitor_node` (implemented in `src/agents/monitoring.py`) is a terminal node in the LangGraph relay that:
1. Extracts the final executive report from the graph state.
2. Invokes an isolated **Judge LLM** (Gemini Flash) to score the report.
3. Logs the result to a dedicated MLflow experiment: `acras_live_monitoring`.

### 5.2 Metrics Logged per Transaction

For every company risk assessment, the following metrics are recorded in MLflow:

| Metric | Dimension | Threshold |
| :--- | :--- | :---: |
| `live/relevance` | Response quality vs. query | ≥ 4 |
| `live/faithfulness` | Grounding in retrieved data | ≥ 4 |
| `live/tool_usage` | Correct tool selection | ≥ 4 |
| `live/business_value` | Strategic KPI alignment | ≥ 3 |

### 5.3 Trace Correlation

Every MLflow monitoring run is tagged with the **`trace_id`** from OpenTelemetry. This creates a bi-directional link between the qualitative score in MLflow and the technical execution waterfall in the OTel backend (e.g., Jaeger), enabling deep-dive debugging of poor-performing reports.
